...
def get_Rt_loss_notsym(
    E_ests_layers, K1s_cpu, K2s_cpu, x1_cpu, x2_cpu, delta_Rtijs_4_4_cpu, qs_cam, ts_cam,
    device='cpu'
):
    """ losses from essential matrix and ground truth R,t
    params:
...
def get_all_loss_DeepF(
    ...
    # print("pts1_eval shape and range: ", pts1_eval.shape, pts1_eval[...,:2].min(), pts1_eval[...,:2].max(), flush=True)
    # print("pts2_eval shape and range: ", pts2_eval.shape, pts2_eval[...,:2].min(), pts2_eval[...,:2].max(), flush=True)
...
def get_all_loss_DeepF_notsym(
    outs, pts1_virt_ori, pts2_virt_ori, K1s, K2s, loss_params, get_residual_summaries=True
):
    """ get f-loss
    params:
        outs: output from deepF,
    ...
    for iter in range(loss_params["depth"]):
        ...
        E_ests_layers.append(
            K2s.transpose(1, 2) @ T2.permute(0, 2, 1) @ out_layers[iter] @ T1 @ K1s
        )
    ...
    E_ests = K2s.transpose(1, 2) @ F_ests @ K1s
...
def val_rt_notsym(
    ...
    """ from essential matrix, get Rt, and error
    params:
        K1_np, K2_np: intrinsic matrices
        x1_single_np: matching point x1
        x2_single_np: matching point x2
        E_est_np, E_gt_np: essential matrix
        F_est_np, F_gt_np: fundamental matrix
    ...
    # _, error_Rt_estW = utils_F._E_to_M(E_est.detach(), K, x1_single_np, x2_single_np, w>0.5, \
    #     delta_Rtij_inv, depth_thres=500., show_debug=False, show_result=False, method_name='Est ws')
    M_estW, error_Rt_estW = utils_F.goodCorr_eval_nondecompose_notsym(
        x1_single_np,
        x2_single_np,
        E_est_np.astype(np.float64),
        delta_Rtij_inv,
    ...
    M_gt, error_Rt_gt = utils_F.goodCorr_eval_nondecompose_notsym(
        x1_single_np,
        x2_single_np,
        E_gt_np.astype(np.float64),
        delta_Rtij_inv,
    ...
    # Evaluating with OpenCV 5-point
    if if_opencv:
        M_opencv, error_Rt_opencv, _, E_return = utils_opencv.recover_camera_opencv_notsym(
            K1_np,
            K2_np,
            x1_single_np,
            x2_single_np,
        ...
        if five_point:
            E_recover_opencv = E_return
            F_recover_opencv = utils_F.E_to_F_np_notsym(E_recover_opencv, K1_np, K2_np)
        else:
            E_recover_opencv, F_recover_opencv = E_return[0], E_return[1]
        # print('+++', K_np)
        epi_dist_mean_opencv, _, _ = utils_F.epi_distance_np(
...
##### superpoint helping functions
def get_matches_from_SP_withsize(imgs_grey, im_size, net_SP, SP_processer, SP_tracker, out_num_points=1000):
    """ get matching from superpoint
    params: 
        imgs_grey: images
        net_SP: superpoint network
    ...
    for idx, img12_grey_float in enumerate(imgs_grey_float):
        outs = net_SP(
            img12_grey_float.unsqueeze(-1).permute(0, 3, 1, 2), im_size[idx]
        )  # [batch_size, 1, H, W]
        # outs = net_SP.process_output(outs, SP_processer)
        outs = process_SP_output_withsize(outs, f(im_size[idx]), SP_processer)
        xs_SP.append(outs["pts_int"])
        deses_SP.append(outs["pts_desc"])
        reses_SP.append(outs["pts_offset"])
    ...
    for batch_idx in range(batch_size):
        ...
        if provide_gt_match_info:
            ...
            for i in range(2):
                ...
                pts_m.append(x)
                pts_m_res.append(res)
            pts_m = torch.cat((pts_m[0], pts_m[1]), dim=1).detach().cpu().numpy()
            pts_m_res = torch.cat((pts_m_res[0], pts_m_res[1]), dim=1).detach().cpu().numpy()
            pts_m += pts_m_res
            F, geometric_matches = cv2.findFundamentalMat(pts_m[:, :2], pts_m[:, 2:], cv2.FM_RANSAC, 3.0, 0.99)
            # print("geometric_matches: ", geometric_matches.shape, geometric_matches.sum(), geometric_matches)
            matching_mask = matching_mask[:, geometric_matches[:,0].astype(bool)]
...
def process_SP_output(output, sp_processer):
    ...
    desc = output['desc']
    # flatten
    semi = set_nan2zero(semi, "Detector")
    desc = set_nan2zero(desc, "Descriptor")
...
def process_SP_output_withsize(output, sz, sp_processer):
    # self.print_p()
    """ # superpoint get sparse points and residuals
    input:
        N: number of points
    ...
    heatmap = flattenDetection(semi) # [batch_size, 1, H, W]
    # nms
    heatmap_nms_batch = sp_processer.heatmap_to_nms(heatmap, sz, tensor=True)
    # extract offsets
    outs = sp_processer.pred_soft_argmax(heatmap_nms_batch, heatmap)
    residual = outs['pred']
    # extract points
